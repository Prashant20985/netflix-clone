...
function Modal() {
  ...
  return (
    ...
    >
        ...
        <ReactPlayer
            url={`https://www.youtube.com/watch?v=${trailer || `cp0u9xHBZJ4`}`}
            width="100%"
            height="100%"
          ...
          <div className="absolute bottom-10 flex w-full items-center justify-between px-10">
            ...
            <button className="modalButton" onClick={() => setMuted(!muted)}> {muted ? <VolumeOffIcon className="h-6 w-6"/> : <VolumeUpIcon className="h-6 w-6"/>}
            </button>
          </div>
        </div>
          ...
          <div className="space-y-6 text-lg">
            ...
            <div className="flex flex-col gap-x-10 gap-y-4 font-light md:flex-row">
              ...
              <div className="flex flex-col space-y-3 text-sm">
                <div>
                  <span className="text-[gray]">Genres: </span> {genres.map((g) => g.name).join(', ')}
                </div>
                <div>
